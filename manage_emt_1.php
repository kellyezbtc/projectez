...
{
	$squp="update emt_tb set bank_name='".addslashes($_REQUEST['bank_name'])."',date_deposit='".date('Y-m-d')."',ad_id='".$_SESSION['uid']."' where emt_id='".$_REQUEST['eid']."'";
...
$sqs="select * from emt_tb,user_tb where user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY ac_date asc";
...
$sqs="select * from emt_tb,user_tb where user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY ac_date asc";
	...
	if($_REQUEST['bank_dep']!='all' and $_REQUEST['amount']!='all')
	{
$sqs="select * from emt_tb,user_tb where bank_dep='".$_REQUEST['bank_dep']."' and amount='".$_REQUEST['amount']."' and  user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY ac_date asc";
	}
	if($_REQUEST['bank_dep']!='all' and $_REQUEST['amount']=='all')
	{
$sqs="select * from emt_tb,user_tb where bank_dep='".$_REQUEST['bank_dep']."' and  user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY ac_date asc";
	}
	if($_REQUEST['bank_dep']=='all' and $_REQUEST['amount']!='all')
	{
$sqs="select * from emt_tb,user_tb where  amount='".$_REQUEST['amount']."' and  user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY ac_date asc";
	}
...
if(mysql_num_rows($res1)>0)
{
                     ...
                     <?php
					 $sqp1="select sum(amount) as stot from emt_tb,user_tb where user_tb.us_id=emt_tb.eu_id and bank_name='' ORDER BY emt_id asc";
...
while($rwck2=mysql_fetch_array($resck2))
{
	$amt=$amt+$rwck2['amount'];
					...
					*/
					 $sqp1="select sum(amount) as stot1 from emt_tb,user_tb where user_tb.us_id=emt_tb.eu_id and bank_name='' and bank_dep='CCEC' ORDER BY emt_id asc";
					 $resp1=mysql_query($sqp1);
					 if(!$resp1)
					 {
						 die('error-1');
					 ...
					 echo '$'.number_format($st2,2,'.',',');
					  ...
					  ?></td>
                                                  ...
                                                  <tr>
                                                    ...
                                                    <td width="83" align="center"><strong>Dep Link&nbsp;</strong></td>
                                                    <td width="197" align="center"><strong>Select Bank</strong></td>
                                                    <td width="185" align="center"><strong>DEPOSITED</strong></td>
                                                    <td width="292" align="center"><strong>Previous</strong></td>
  ...
  {
  ?>
     <form name="frm1" action="" method="post">                                             <tr>
       <td align="center"><?php 
...
<?php
	...
	if($rwc['bank_name']=='')												
	{												?>
                                                    <input type="submit" name="Submitd" id="Submitd" value="DEPOSITED" style="background-color: #008CBA; color: white;">
                                                    <?php
	}
	else
	{
	 ...
	 ?>                                               </td>
                                                    <td align="center"><?php
$sqpr="select * from emt_tb,user_tb where user_tb.us_id=emt_tb.eu_id and bank_name!='' and us_id='".$rwc['us_id']."' and emt_id!='".$rwc['emt_id']."' ORDER BY date_deposit desc limit 0,5";	
...
echo $arr[2].'-'.$arr[1].'-'.$arr[0].' '.$rwpr['bank_name'].'<br>';
	  ...
	  echo '( $'.$rwpr['amount']." , ".$rwpr['bank_name'].') <br>';
                                                    ...
                                                    <td height="30" align="center"><a href="manage_emt.php?delid=<?php echo $rwc['emt_id'];?>">X</a></td>
                                                  </tr>
       </form>                                           <?php 
  }
  ?>
